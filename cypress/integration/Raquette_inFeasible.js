...
describe('filesuploading', ()=>
{
    ...
    it('entering schdule name' ,()=> {
        ...
        cy.get('input[type="text"]').type("RaquetteinFeasible_".concat(date))
        cy.get('input[type="text"]').type('{enter}')
        cy.wait(2000)
        const networkFile= 'Raquette_inFeasible/NetworkDescription.xlsx'
        const efficienceyFile= 'Raquette_inFeasible/GeneratorEfficiency.xlsx'
        const conversionFile= 'Raquette_inFeasible/StorageConversion.xlsx'
       ...
       // cy.get('div[role="listbox"]>:first-child>:nth-child(3)>:first-child').contains('Normal').click()
        ...
        cy.get('a[download^="aom_RaquetteinFeasible"]').click()
